...
//命令数の表示
void printInstNum(uint s[]) {
  int i;
  string isa[39] = {"ADD", "SUB", "XOR", "ADDI", "SUBI", "SLLI", "SRAI", "XORI", "FADD",
		   "FSUB", "FMUL", "FMULN", "FINV", "FSQRT", "IMOVF", "FMOVI", "MVLO", "MVHI", "FMVLO",
		   "FMVHI", "J", "BEQ", "BLT", "BLE", "FBEQ", "FBLT", "FBLE", "JR", "CALL", 
		   "CALLR", "RETURN", "LDR", "FLDR", "STI", "LDI", "FSTI", "FLDI", "INPUTB", "OUTPUTB"};
  cerr << "Number of Instructions" << endl;
  for (i = 0; i < 39; i++) 
    cerr << isa[i] << " :\t" << s[i+1] << endl;
  cerr << "OTHERS :\t" << s[0] << endl;
}
