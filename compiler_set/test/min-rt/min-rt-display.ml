...
let rec calc_rgb_element x =
  let ix = int_of_float x in
  if ix > 255 then 255 else if ix < 0 then 0 else ix
in

let rec write_rgb _ =
  let r = calc_rgb_element rgb.(0) in (* Red   *)
  let g = calc_rgb_element rgb.(1) in (* Red   *)
  let b = calc_rgb_element rgb.(2) in (* Red   *)
  draw (r * 65536 + g * 256 + b)	      (* encode into 24bit integer *)
in
