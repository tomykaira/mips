...
and read_only' x = function
  | Unit | Nil | Int(_) | Float(_) | ExtArray(_) | Neg(_) | FNeg(_) | Sll(_, _) | Sra(_, _) | Add(_, _) | Sub(_, _) | Mul(_, _) | FAdd(_, _) | FSub(_, _) | FMul(_, _) | FDiv(_, _) | Get(_, _) | GetTuple(_,_) | PutTuple(_,_,_) -> true
  | Var(y) | Put(_,_,y) -> y <> x
  | Cons(y,z) -> x <> y && x <> z
...
let rec call_leaf = function
  | MakeCls(_,_,e) | Let(_, _, e) | LetTuple(_,_,e) | LetList(_,_,e)  -> call_leaf e
...
(* 関数用 *)
let h { name = _; args = yts; formal_fv = _; body = e } =
  call_leaf e;
  g e
