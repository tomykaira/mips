...
let rec iter n e = 
  ...
  if n = 0 then e else
  let e' =  off offel Elim.f (off offte IfThenElse.f (off offin (Inline.f (!limit-n)) (off offcf ConstFold.f  (off2 (!offaa || (n mod 2 <> 0)) AliasAnalysis.f ( (off offbe Beta.f (off offcs Cse.f e))))))) in
